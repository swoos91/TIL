...
ls() #전체 객체 리스트 보여주는 함수 

summary(x2) # 열 단위로 summary
...
?apply #apply(matrix, 1or2, function, something related to function)

apply(x2, 2, max)
apply(x2, 2, min)
apply(x2, 2, mean)

a1 <- array(1:30, dim=c(2,3,5)) # array 생성 함수
a1[1,3,4]
a1[,,3]
a1[1,,]
a1[,2,]
...
btype <- factor(c("A", "O", "AB", "B", "O", "A"), levels=c("A","B","O","AB"))
btype
summary(btype)
levels(btype)

gender <- factor(c(1,2,1,1,1,2,1,2), levels=c(1,2), labels=c("남성","여성"))  #labels
gender
summary(gender)
levels(gender)

data()
iris; head(iris); tail(iris) #head 와 tail 은 일부만 출력할 때 사용
View(iris)
str(iris)
...
fruit[,3]
fruit$qty # col 하나만 가능
fruit[[3]]
fruit[3]  # 데이터프레임 형식 유지

str(fruit$qty)  #fruit$qty 벡터 구조
str(fruit[3])   #fruit[3] 데이터프레임 형식 유지
...
colnames(df_midterm)
rownames(df_midterm)
names(df_midterm) # names = colnames
...
df$var_mean <- df$var_sum/2     # df에 파생변수 생성
...
#csv파일열기
score <- read.csv("data/score.csv")#상대패스 # 첫 행은 colnames로 인식 // 절대패스(위치장소의 절대주소)
score
str(score)
score$sum <- 
...
summary(score$result) #팩터 인식x --> 문자형으로 인식하므로 summary 원하는 값 안나옴
table(score$result) #팩터형이 아니더라도 무조건 집계 내줌
...
emp <- read.csv(file.choose(),
                stringsAsFactors = F) #문자열 팩터X default=TRUE
emp
str(emp)
