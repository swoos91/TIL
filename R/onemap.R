...
head(DONG)
#View(korpopmap3@data)  DONG korpopmap3
data(korpopmap3)
...
korpopmap3@data<-korpopmap3@data[c(407:424),] 
korpopmap3@polygons<-korpopmap3@polygons[c(407:424)] 

korpopmap3@data$name<-gsub('·','',korpopmap3@data$name) 
colnames(DONG)<-c('구별','name','일인가구')
dong<- DONG %>%filter(구별=='강동구')

korpopmap3@data<-merge(korpopmap3@data,dong,by.x='name',sort=FALSE)  #merge: join과 비슷한 기능 
